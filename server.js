...
function gateKeeper(req, res, next) {
  const userAndPassString = req.get('x-username-and-password');
  const userAndPassObject = queryString.parse(userAndPassString);
...
//   if (searchUsers(userAndPassObject.user)) {
//       req.user = USERS[searchUsers(userAndPassObject.user)];
//   };
  
  function searchUsers(user) {
    return user.userName === userAndPassObject.user;
  };
  
  if (USERS.find(searchUsers)) {
      req.user = (USERS.find(searchUsers));
  };
